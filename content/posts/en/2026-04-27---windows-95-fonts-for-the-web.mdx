...
There are plenty of fonts out there that look like MS Sans Serif. [W95FA](https://www.dafont.com/w95fa.font) is one of them, made to remember that era. And they work, from a distance. Up close, they're not it. They're not pixel perfect.
...
A library called [`monobit`](https://pypi.org/project/monobit/) reads old `.FON` files and extracts the glyph data. From there, [`fontTools`](https://pypi.org/project/FONtools/) rebuilds each size as a proper TTF, with correct metrics and the `gasp` table configured so browsers wouldn't apply antialiasing and ruin the pixels. Then [`brotli`](https://pypi.org/project/brotli/) compresses to WOFF2 and embeds the result as base64 directly in the CSS.
...
At first it seems strange. Then there's no other way to think about it.

## The package

The result was `@react95/fonts`: four variants, MS Sans Serif and MS Serif, at 96 dpi and 120 dpi, six sizes each. Twenty-eight CSS files generated automatically by the Python script, with base64 embedded, no extra network requests.
...
The script still prints a ready-to-paste `exports` block for `package.json` at the end of each run. Nothing's edited by hand.
