---
title: 'Oi'
template: 'page'
slug: '/sobre'
description: 'Quem é Gabriel Daltoso: desenvolvedor frontend há mais de uma década, criador do React95, e o que ele faz longe da tela'
---

Meu nome é **Gabriel Daltoso**. Sou desenvolvedor frontend há mais de uma década e trabalho construindo interfaces e sistemas para a web.

Ao longo da minha carreira, tive a oportunidade de trabalhar em diferentes tipos de produtos, desde ferramentas para desenvolvedores até plataformas usadas por milhares de pessoas diariamente. Gosto especialmente de trabalhar na arquitetura de interfaces, na criação de design systems e em ferramentas que tornam o desenvolvimento mais consistente e produtivo.

Se você prefere isso em formato de tópicos, com empresas e datas, está tudo no [meu currículo](/cv).

Também sou criador e mantenedor do projeto open source [React95](/react95-fun-side-project-of-the-year/), uma biblioteca de componentes para React inspirada na interface clássica do Windows 95.

Eu gosto de entender como as coisas funcionam por dentro e de montá-las peça por peça. Isso aparece tanto no meu trabalho quanto nos meus hobbies

### Relógios

<span class="figure float-left no-padding-left">

![2 exemplares construídos por mim. No logo "GG" e "Fun is the key" como mensagem](/media/sobre-mim/relogio.jpg 'Relógios.png | 250')

</span>

Gosto de montar meus próprios relógios, escolhendo cada componente com cuidado, como a caixa, o mostrador, os ponteiros e o tipo de pulseira, criando assim combinações únicas que tenham personalidade. Esse processo me atrai porque mistura precisão, estética e paciência, como se cada peça tivesse um papel específico até o conjunto finalmente fazer sentido.

<br class="hide-sm"/>

### Miçangas

<span class="figure float-right no-padding-right">

![Guitarra, flores, abacaxis e um trabalho em andamento](/media/sobre-mim/micanga.jpg 'Miçangas.png | 280')

</span>

É uma atividade manual e detalhista que transforma pixel art em algo físico, alinhando pequenas peças como se cada uma fosse um pixel.

É um passatempo que me faz muito bem, porque me ajuda a sair um pouco do ritmo acelerado do dia a dia. Quando estou focado nisso, consigo me afastar das redes sociais e das telas por mais tempo, relaxar a mente e aproveitar um momento mais calmo e presente.

<br class="hide-sm"/>

No fundo, todas essas coisas são variações do mesmo processo: organizar pequenas partes com cuidado até que algo interessante apareça.

Este blog é um espaço para compartilhar ideias, projetos, experimentos e o que estou aprendendo pelo caminho.
